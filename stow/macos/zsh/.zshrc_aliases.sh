...
alias glls='git log --graph --pretty="%Cred%h%Creset -%C(auto)%d%Creset %s %Cgreen(%ar) %C(bold blue)<%an>%Creset" --all --simplify-by-decoration'
alias glf='git log --oneline --decorate --graph --first-parent'
alias gllf='git log --graph --pretty="%Cred%h%Creset -%C(auto)%d%Creset %s %Cgreen(%ar) %C(bold blue)<%an>%Creset" --first-parent'
alias p='git pull'
alias gp='git push'
